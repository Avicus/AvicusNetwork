...
import net.avicus.hook.utils.ReflectionUtils.PackageType;
...
public enum ParticleEffect {
   ...
   * brewing stands and monster spawners: <ul> <li>It looks like a little gray cloud <li>The speed
   * value influences the velocity at which the particle flies off </ul>
   */
   ...
   * something: <ul> <li>It looks like a white swirl <li>The speed value causes the particle to only
   * move upwards when set to 0 <li>Only the motion on the y-axis can be controlled, the motion on
   * the x- and z-axis are multiplied by 0.1 when setting the values to 0 </ul>
   */
  SPELL("spell", 13, -1),
  /**
   * A particle effect which is displayed when instant splash potions hit something: <ul> <li>It
   * looks like a white cross <li>The speed value causes the particle to only move upwards when set
   * to 0 <li>Only the motion on the y-axis can be controlled, the motion on the x- and z-axis are
   * multiplied by 0.1 when setting the values to 0 </ul>
   */
  SPELL_INSTANT("instantSpell", 14, -1),
  /**
   * A particle effect which is displayed by entities with active potion effects: <ul> <li>It looks
   * like a colored swirl <li>The speed value causes the particle to be colored black when set to 0
  ...
  /**
   * A particle effect which is displayed by witches: <ul> <li>It looks like a purple cross <li>The
   * speed value causes the particle to only move upwards when set to 0 <li>Only the motion on the
   * y-axis can be controlled, the motion on the x- and z-axis are multiplied by 0.1 when setting
   * the values to 0 </ul>
   */
  ...
  /**
   * A particle effect which is displayed by nether portals, endermen, ender pearls, eyes of ender,
   * ender chests and dragon eggs: <ul> <li>It looks like a purple cloud <li>The speed value
   * influences the spread of this particle effect </ul>
   */
   ...
   * redstone repeaters: <ul> <li>It looks like a tiny colored cloud <li>The speed value causes the
   * particle to be colored red when set to 0 </ul>
   */
  ...
  /**
   * Displays a particle effect which is only visible for all players within a certain range in the
  ...
  public void display(float offsetX, float offsetY, float offsetZ, float speed, int amount,
    ...
    new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, range > 256, null)
        .sendTo(center, range);
  }
  ...
  /**
   * Displays a single particle which flies into a determined direction and is only visible for all
  ...
  /**
   * Displays a single particle which flies into a determined direction and is only visible for the
   * specified players
   *
   * @param direction Direction of the particle
   * @param speed Display speed of the particle
   ...
   * @param range Range of the visibility
   * @throws ParticleVersionException If the particle effect is not supported by the server version
   * @throws ParticleDataException If the particle effect does not require additional data or if the
  ...
  public void display(ParticleData data, float offsetX, float offsetY, float offsetZ, float speed,
    ...
    new ParticlePacket(this, offsetX, offsetY, offsetZ, speed, amount, range > 256, data)
        .sendTo(center, range);
  }

  /**
   * Displays a particle effect which requires additional data and is only visible for the specified
   * players
   *
   * @param data Data of the effect
   * @param offsetX Maximum distance particles can fly away from the center on the x-axis
   ...
   * ParticleEffect#BLOCK_CRACK} and {@link ParticleEffect#BLOCK_DUST} <p> This class is part of the
   ...
   * ParticleEffect#BLOCK_DUST} effects <p> This class is part of the <b>ParticleEffect Library</b>
   * and follows the same usage conditions
   *
   * @author DarkBlade12
   * @since 1.6
   */
  ...
  /**
   * Represents the color for effects like {@link ParticleEffect#SPELL_MOB}, {@link
   * ParticleEffect#SPELL_MOB_AMBIENT} and {@link ParticleEffect#NOTE} <p> This class is part of the
   * <b>ParticleEffect Library</b> and follows the same usage conditions
   *
   * @author DarkBlade12
   * @since 1.7
  ...
  /**
   * Represents a runtime exception that is thrown either if the displayed particle effect requires
   * data and has none or vice-versa or if the data type is incorrect <p> This class is part of the
   * <b>ParticleEffect Library</b> and follows the same usage conditions
   *
   * @author DarkBlade12
   * @since 1.6
  ...
  /**
   * Represents a runtime exception that is thrown if the displayed particle effect requires a newer
   * version <p> This class is part of the <b>ParticleEffect Library</b> and follows the same usage
   * conditions
   *
   * @author DarkBlade12
   * @since 1.6
  ...
  public static final class ParticlePacket {
    ...
    public ParticlePacket(ParticleEffect effect, ParticleColor color, boolean longDistance) {
      this(effect, color.getValueX(), color.getValueY(), color.getValueZ(), 1, 0, longDistance,
          null);
     ...
     * {@link #sendPacket} and sets {@link #initialized} to <code>true</code> if it succeeds <p>
     * <b>Note:</b> These fields only have to be initialized once, so it will return if {@link
     * #initialized} is already set to <code>true</code>
     *
    ...
    public static void initialize() throws VersionIncompatibleException {
      ...
      try {
        ...
        getHandle = ReflectionUtils
            .getMethod("CraftPlayer", PackageType.CRAFTBUKKIT_ENTITY, "getHandle");
     ...
     *
     * @param center Center location of the effect
     * @param range Range in which players will receive the packet (Maximum range for particles is
    ...
    /**
     * Represents a runtime exception that is thrown if a bukkit version is not compatible with this
     * library <p> This class is part of the <b>ParticleEffect Library</b> and follows the same
     * usage conditions
     *
     * @author DarkBlade12
     * @since 1.5
